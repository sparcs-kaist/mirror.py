...
>
> Since 1.2, the default rsync flags are `-vrltDSH` (previously `-vrlptDSH`):
> the `-p` (preserve permissions) flag was dropped, so upstream file modes are
> no longer mirrored. This is not configurable.
